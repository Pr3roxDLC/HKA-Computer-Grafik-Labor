...
FLOAT_TYPE Vector<FLOAT_TYPE, N>::length() const {
  return sqrt(this->square_of_length());
}

template <class FLOAT_TYPE, size_t N>  
FLOAT_TYPE Vector<FLOAT_TYPE, N>::square_of_length() const {
  FLOAT_TYPE square_of_length = 0.0;
  for (size_t i = 0u; i < N; i++) {
    square_of_length += this->vector[i] * this->vector[i];
  }
...
template <class FLOAT_TYPE, size_t N>    
FLOAT_TYPE operator*(const Vector<FLOAT_TYPE, N> vector1, const Vector<FLOAT_TYPE, N> vector2) {
